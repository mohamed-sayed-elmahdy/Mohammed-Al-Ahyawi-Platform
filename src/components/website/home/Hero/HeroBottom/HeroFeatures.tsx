...
export default function HeroFeatures() {
  return (
    <div
      ...
      className="max-w-11/12 mx-auto rounded-2xl border border-(--color-accent)/75 bg-black/55 p-3 sm:p-3"
    >
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-5">
        {features.map(({ title, icon: Icon }, index) => (
          <div
            key={title}
            className="group flex items-center gap-3 rounded-2xl border border-transparent  px-4 py-3 transition duration-300 hover:-translate-y-1 hover:border-amber-300/35 hover:bg-amber-400/10"
          >
